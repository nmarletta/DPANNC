...
public class HYexperiments {
    ...
    public static void exp1() throws Exception {
        ...
        Progress.newBar("Experiment " + name, (2 + 3 * reps) * dimensions.length);
        int pg = 0;
        ...
        try (FileWriter writer = new FileWriter(filepathTarget.toAbsolutePath().toString())) {
            ...
            for (int d : dimensions) {
                ...
                // populate HY
                hy.populateFromDB(n, d, table1, db);
                Progress.updateBar(++pg);
                ...
                for (int i = 0; i < reps; i++) {
                    ...
                    Result BRUTEres = new Result().loadDistancesBetween(q, table1, db);
                    Result HYres = new Result().loadDistancesBetween(q, queryList, table1, db);
                    Progress.updateBar(++pg);

                    Progress.newStatus("writing results...");

                    // INNER REGION results
                    Set<String> B_inner = BRUTEres.lessThan(radius); // initial r for raw dataset
                    Set<String> A_inner = HYres.lessThan(radius); // HYs r for transformed dataset
                    ...
                    Set<String> B_fuzzy = BRUTEres.within(radius, alpha * radius);
                    Set<String> A_fuzzy = HYres.within(radius, alpha * radius);
